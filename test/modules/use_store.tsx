...
import {API, API2, SWAP1, SWAP2, AppSingleWithoutSelector, AppSingleWithSelector, AppMultipleWithoutSelector, AppMultipleWithSelector, AppWithSwappedStore} from '../fixtures/app';
...
describe ( 'useStore', it => {
  ...
  it.serial ( 'works with swapped stores', async t => {
    ...
    const app = mount ( React.createElement ( AppWithSwappedStore, {rendering} ) ),
          getText = selector => app.find ( selector ).text (),
          getValue = () => getText ( '#value' ),
          click = selector => app.find ( selector ).simulate ( 'click' );
